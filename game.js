/*
 * Seed of Life - v0.4 Wakfu Cohesive Prototype
 * Pure JS with TS-like comments to ease future migration.
 */

const GAME_VERSION = "0.4.0-ui-inventory-intro";
...
class ProfessionSystem {
  constructor(scene, persisted = null) {
    this.scene = scene;
    this.professions = { Lumberjack: persisted?.Lumberjack || { xp: 0, level: 1 } };
  }
...
class ParallaxBackgroundManager {
  ...
  createTextures() {
    ...
    g.fillStyle(0x4b7f58, 1);
    g.fillRect(0, 0, 1024, 1024);

    // Cleaner ground: subtle grass variation + natural soil patches.
    for (let i = 0; i < 1800; i += 1) {
      const col = [0x4e8659, 0x5a9161, 0x447a52][Phaser.Math.Between(0, 2)];
      g.fillStyle(col, Phaser.Math.FloatBetween(0.1, 0.55));
      g.fillCircle(Phaser.Math.Between(0, 1024), Phaser.Math.Between(0, 1024), Phaser.Math.Between(1, 3));
    }
    for (let i = 0; i < 65; i += 1) {
      g.fillStyle(0x6a4a35, Phaser.Math.FloatBetween(0.14, 0.26));
      g.fillEllipse(Phaser.Math.Between(0, 1024), Phaser.Math.Between(0, 1024), Phaser.Math.Between(45, 130), Phaser.Math.Between(25, 70));
    }
    g.generateTexture("td_ground", 1024, 1024);
    g.clear();

    g.fillStyle(0x98d7ce, 1);
    g.fillRect(0, 0, 1024, 512);
    g.fillStyle(0x40644e, 0.75);
    g.beginPath();
    g.moveTo(0, 512);
  ...
  createLayers() {
    ...
    // Layer 2 gameplay = managed by MainWorld with depth sorting Y.
...
class CombatSystem {
  constructor(scene, player) {
    ...
    this.player = player;
    this.lastAttack = 0;
    this.attackSpeedMs = 580;
    this.skillCooldowns = Array.from({ length: 8 }, () => 0);
  }

  update(time, monsters) {
    const target = monsters.getChildren().find((m) => m.active && Phaser.Math.Distance.Between(m.x, m.y, this.player.x, this.player.y) <= this.player.autoAttackRange);
    if (target && time - this.lastAttack > this.attackSpeedMs) {
      this.lastAttack = time;
      target.hp -= 10;
      this.scene.events.emit("floating:text", "-10", 0xff8c8c, target.x, target.y - 26);
      if (target.hp <= 0) {
        ...
        this.scene.events.emit("floating:text", `+${ecoGain} Eco 🌱`, 0xa2ed8d, target.x, target.y - 40);
  ...
  castSkill(index, time) {
    ...
    this.skillCooldowns[index] = time + (1300 + index * 180);
    this.scene.events.emit("floating:text", `Skill ${index + 1}`, 0x9dd8ff);
    return true;
  }
}
...
class IntroScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(guardianBtn.x, guardianBtn.y, "Je suis le Gardien", { fontSize: "28px", color: "#f4ffe6" }).setOrigin(0.5).setAlpha(0).setDepth(40).setName("btnText");

    const btnText = this.children.getByName("btnText");

    let introSparklesActive = false;
    this.time.addEvent({
      delay: 120,
      loop: true,
      callback: () => {
        if (!introSparklesActive) return;
    ...
    this.time.delayedCall(9000, () => {
      this.tweens.add({ targets: playerEcho, alpha: 1, scale: 1, duration: 1000 });
    });

    this.time.delayedCall(10300, () => {
      this.tweens.add({ targets: [guardianBtn, btnText], alpha: 1, duration: 700 });
    });
...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const sky = this.add.rectangle(w * 0.5, h * 0.5, w, h, 0x102918);
    ...
    this.tweens.add({ targets: [crown, crown2, crown3], scaleX: 1.03, scaleY: 1.03, yoyo: true, duration: 3000, repeat: -1, ease: "Sine.InOut" });
    ...
    this.add.text(w * 0.5, h * 0.2, "Seed of Life", {
      fontSize: "72px",
      color: "#f1ffd8",
    ...
    const label = this.add.text(btn.x, btn.y, "Nouvelle Graine", {
      fontSize: "34px",
      color: "#f9ffea",
      fontFamily: "Georgia, serif"
    }).setOrigin(0.5);
    ...
    // keep refs to avoid tree-shaking style lint confusion
    this._menuNodes = [sky, trunk, crown, crown2, crown3, label];
  }
}
...
class MainWorldScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Start progression: no axe, very few seeds.
    this.inventory = save?.inventory || { Graines: Phaser.Math.Between(3, 5), Branches: 0, Pierres: 0, Bois: 0, "Wooden Axe": 0, "Monster Essence": 0 };
    this.selectedItem = null;
    this.isPlantingMode = false;

    this.bgManager = new ParallaxBackgroundManager(this, this.worldWidth, this.worldHeight);

    // Layer 2 gameplay group
    ...
    this.player.setAxeVisible((this.inventory["Wooden Axe"] || 0) > 0);
    ...
    this.gatherables = this.physics.add.group({ classType: Phaser.Physics.Arcade.Sprite, maxSize: 60 });
    this.spawnStartingGroundResources();

    this.monsters = this.physics.add.group({ classType: Phaser.Physics.Arcade.Sprite, maxSize: 50 });
    ...
    this.events.on("inventory:consume", (item, qty) => this.consumeInventory(item, qty));
  ...
  createTextures() {
    if (!this.textures.exists("tree_placeholder")) {
      ...
      g.fillStyle(0x1a1a1a, 0.22); g.fillEllipse(44, 84, 56, 18);
      g.fillStyle(0x6d4022, 1); g.fillRoundedRect(36, 40, 16, 40, 4);
      g.fillStyle(0x2d8e4e, 1); g.fillCircle(44, 34, 28);
      g.fillStyle(0x46ad65, 0.8); g.fillCircle(32, 32, 14); g.fillCircle(55, 30, 12);
    ...
    if (!this.textures.exists("sapling_placeholder")) {
      ...
      g2.fillStyle(0x5c361f, 1); g2.fillRect(20, 18, 4, 16);
      g2.fillStyle(0x59b155, 1); g2.fillCircle(22, 14, 8);
    ...
    if (!this.textures.exists("axe_icon")) {
      ...
      axe.fillStyle(0x8f6b47, 1); axe.fillRect(8, 6, 3, 20);
      axe.fillStyle(0xb6c4d0, 1); axe.fillTriangle(11, 9, 20, 5, 20, 16);
      axe.generateTexture("axe_icon", 24, 30);
      axe.destroy();
    }

    if (!this.textures.exists("monster_placeholder")) {
      const gm = this.make.graphics({ add: false });
      gm.fillStyle(0xc95d58, 1); gm.fillRoundedRect(4, 4, 24, 24, 6);
      gm.generateTexture("monster_placeholder", 32, 32);
      gm.destroy();
    }

    if (!this.textures.exists("branch_placeholder")) {
      const gb = this.make.graphics({ add: false });
      gb.fillStyle(0x7a4e2c, 1);
      gb.fillRoundedRect(2, 10, 24, 4, 2);
      gb.generateTexture("branch_placeholder", 28, 24);
      gb.destroy();
    }

    if (!this.textures.exists("stone_placeholder")) {
      const gs = this.make.graphics({ add: false });
      gs.fillStyle(0x8f959d, 1);
      gs.fillCircle(11, 11, 9);
      gs.generateTexture("stone_placeholder", 24, 24);
      gs.destroy();
    }

    if (!this.textures.exists("plant_marker")) {
      const pm = this.make.graphics({ add: false });
      pm.fillStyle(0x4f2f1f, 0.6);
      pm.fillEllipse(20, 12, 34, 12);
      pm.generateTexture("plant_marker", 40, 24);
      pm.destroy();
    }
  }
  ...
  spawnOrganicTrees() {
    ...
    while (points.length < count) {
      const x = Phaser.Math.Between(250, this.worldWidth - 250);
      const y = Phaser.Math.Between(220, this.worldHeight - 220);
      const ok = points.every((p) => Phaser.Math.Distance.Between(p.x, p.y, x, y) > minDist);
      if (ok) points.push({ x, y });
    }

    points.forEach((p) => {
      const size = Phaser.Math.Between(1, 3);
      const tree = this.add.image(p.x, p.y, "tree_placeholder").setScale(0.85 + size * 0.16);
      tree.setDataEnabled();
  ...
  spawnStartingGroundResources() {
    ...
    for (let i = 0; i < total; i += 1) {
      ...
      const x = Phaser.Math.Between(160, this.worldWidth - 160);
  ...
  handleWorldClick(pointer) {
    ...
    if (isTouch && pointer.x <= this.scale.width * 0.4) return;

    const wx = pointer.worldX;
    const wy = pointer.worldY;

    // priority: gatherables > tree chop > planting > move
  ...
  startChop(tree) {
    ...
    const axeSwing = this.add.image(this.player.x + 18, this.player.y - 8, "axe_icon");
    this.entityLayer.add(axeSwing);

    this.tweens.add({ targets: axeSwing, angle: { from: -35, to: 55 }, duration: 220, yoyo: true, repeat: 5, ease: "Sine.InOut" });
  ...
  finishChop(tree) {
    ...
    const isPlanted = !!tree.getData("plantedByPlayer");
    const wood = Math.round(baseWood * (isPlanted ? 1.5 : 1));
    const seeds = Phaser.Math.Between(1, 2) + (isPlanted ? 2 : 0);
  ...
  craftWoodenAxe() {
    if (this.player.hasAxe) {
      this.events.emit("floating:text", "Déjà équipée", 0xd6e9bf, this.player.x, this.player.y - 42);
      return false;
    }

    if ((this.inventory.Branches || 0) < 5 || (this.inventory.Pierres || 0) < 2) {
      this.events.emit("floating:text", "Recette: 5 Branches + 2 Pierres", 0xffc59f, this.player.x, this.player.y - 42);
      return false;
    }

    this.consumeInventory("Branches", 5);
    this.consumeInventory("Pierres", 2);
    this.addInventory("Wooden Axe", 1);

    this.player.setAxeVisible(true);
    this.axeVisual.setVisible(true);
    this.events.emit("floating:text", "Wooden Axe équipée !", 0xc5f6a4, this.player.x, this.player.y - 48);
    return true;
  }
  ...
  setupTouchControls() {
    this.stickBase = this.add.circle(110, this.scale.height - 110, 62, 0x000000, 0.3).setScrollFactor(0).setDepth(9000).setVisible(false);
    this.stickThumb = this.add.circle(110, this.scale.height - 110, 34, 0xb4df8f, 0.9).setScrollFactor(0).setDepth(9001).setVisible(false);
  ...
  update(time, delta) {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.I)) {
      this.scene.get("UIOverlay")?.toggleInventory();
    }

    this.player.updateFromInput(cursors, joystick, delta);
    this.axeVisual.setPosition(this.player.x + 16, this.player.y - 8).setVisible(this.player.hasAxe);
...
class UIOverlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    world.events.on("world:ready", (state) => {
      ...
      this.toolText.setText(state.hasAxe ? "Outil: Wooden Axe" : "Outil: Aucun");
    });
  ...
  createHud() {
    ...
    const ecoPanel = this.add.rectangle(18, 18, 360, 130, wood, 0.85)
      .setOrigin(0, 0)
      .setStrokeStyle(3, leaf)
      .setDepth(9500)
      .setScrollFactor(0);

    this.ecoText = this.add.text(34, 33, "EcoBalance: --", { fontSize: "20px", color: "#eef9e1", fontFamily: "Georgia, serif" }).setDepth(9501).setScrollFactor(0);
    this.profText = this.add.text(34, 58, "Lumberjack --", { fontSize: "17px", color: "#d9efc5" }).setDepth(9501).setScrollFactor(0);
    this.toolText = this.add.text(34, 80, "Outil: Aucun", { fontSize: "16px", color: "#e8efdc" }).setDepth(9501).setScrollFactor(0);

    this.ecoBarBg = this.add.rectangle(34, 114, 250, 16, 0x4e2d25, 0.95).setOrigin(0, 0.5).setDepth(9501).setScrollFactor(0);
    this.ecoBarFill = this.add.rectangle(34, 114, 250, 16, 0x6ad76a, 0.98).setOrigin(0, 0.5).setDepth(9502).setScrollFactor(0);

    this.miniMap = this.add.rectangle(this.scale.width - 168, 18, 150, 150, wood, 0.82)
      .setOrigin(0, 0)
      .setStrokeStyle(2, leaf)
      .setDepth(9500)
      .setScrollFactor(0);
    ...
    this.invToggle = this.add.rectangle(18, this.scale.height - 74, 150, 56, wood, 0.9)
      .setOrigin(0, 0)
      .setStrokeStyle(3, leaf)
      .setDepth(9500)
      .setScrollFactor(0)
    ...
    // hotkeys panel
    for (let i = 0; i < 8; i += 1) {
      const btn = this.add.rectangle(this.scale.width - (i + 1) * 70, this.scale.height - 48, 58, 58, 0x36513c, 0.9)
        .setDepth(9501).setScrollFactor(0).setStrokeStyle(2, 0xa7cc80).setInteractive();
  ...
  createInventoryPanel() {
    ...
    this.invPanel = this.add.rectangle(w * 0.5 - 220, h - 390, 440, 360, 0x2b241d, 0.94)
      .setOrigin(0, 0)
      .setStrokeStyle(3, 0x9cc56e)
      .setDepth(9600)
      .setScrollFactor(0)
      .setVisible(false);

    this.invTitle = this.add.text(this.invPanel.x + 18, this.invPanel.y + 12, "Sac du Gardien", { fontSize: "24px", color: "#efffdd", fontFamily: "Georgia, serif" })
      .setDepth(9601)
      .setScrollFactor(0)
      .setVisible(false);

    this.invHint = this.add.text(this.invPanel.x + 18, this.invPanel.y + 320, "Clique sur une Graine pour planter", { fontSize: "14px", color: "#d6efc2" })
      .setDepth(9601)
      .setScrollFactor(0)
      .setVisible(false);
    ...
    this.craftBtn = this.add.rectangle(this.invPanel.x + 312, this.invPanel.y + 292, 108, 46, 0x5d8d4a, 0.95)
      .setStrokeStyle(2, 0xd5f3ad)
      .setDepth(9601)
      .setScrollFactor(0)
      .setInteractive()
      .setVisible(false);
    this.craftLabel = this.add.text(this.craftBtn.x, this.craftBtn.y, "Craft", { fontSize: "20px", color: "#f4ffe6", fontFamily: "Georgia, serif" })
      .setOrigin(0.5)
      .setDepth(9602)
      .setScrollFactor(0)
      .setVisible(false);
    ...
    this.craftBtn.on("pointerdown", () => {
      const world = this.scene.get("MainWorld");
      if (world && world.craftWoodenAxe()) {
        this.toolText.setText("Outil: Wooden Axe");
      }
    });
  }
  ...
  toggleInventory() {
    ...
    this.craftBtn.setVisible(v);
    this.craftLabel.setVisible(v);
  ...
  setEco(value) {
    ...
    this.tweens.add({ targets: this.ecoBarFill, width: 250 * ratio, duration: 240, ease: "Sine.Out" });
